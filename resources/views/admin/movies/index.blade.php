        ...
        <div class="col-md-12">
            ...
            <div class="tile shadow">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <select id="genre" name="genre_id" class="form-control select2" required>
                                <option value="">@lang('site.all') @lang('genres.genres')</option>
                                @foreach($genres as $genre)
                                    <option  {{request()->genre_id==$genre->id?'selected':''}} value="{{$genre->id}}">{{$genre->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <select id="actor" name="actor_id" class="form-control select2" required>
                                <option value="">@lang('site.all') @lang('actors.actors')</option>
                                @if( $actor)
                                    <option  {{$actor->id==request()->actor_id?'selected':''}} value="{{$actor->id}}">{{$actor->name}}</option>
                                @endif
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <select id="type" name="type" class="form-control select2" required>
                                ...
                                @foreach(['now_playing','upcoming','popular'] as $type)
                                    <option  {{$type==request()->type?'selected':''}} value="{{$type}}">@lang('movies.'.$type)</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
    ...
    <script>
        ...
        let moviesTable = $('#movies-table').DataTable({
            ...
            columns: [
                ...
                {data: 'poster', name: 'poster',width:'10%', searchable: false},
